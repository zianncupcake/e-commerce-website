import { Row, Col, Table, Button } from "react-bootstrap";
import { LinkContainer } from "react-router-bootstrap";
import AdminLinksComponent from "./AdminLinksComponent";

import { useState, useEffect } from "react";

const AdminUsersPageComponent = ({ fetchUsers, deleteUser }) => {
  const [users, setUsers] = useState([]);

  //handle side effects in functional components. code not directly related to rendering of component but has impact on component's behaviour or external environment
  //fetch users when component mounts 
  useEffect(() => {
    fetchUsers()
    .then(res => setUsers(res))
    .catch((er) =>
    console.log(
        er.response.data.message ? er.response.data.message : er.response.data
      )
      );
 },[])

 const deleteHandler = async (userId) => {
    if (window.confirm("Are you sure?")) {
        const data  = await deleteUser(userId);
        const updatedUsers = users.filter((user) => user._id !== userId);
        setUsers(updatedUsers);

    }
  };


  return (
    <Row className="m-5">
      <Col md={2}>
        <AdminLinksComponent />
      </Col>
      <Col md={10}>
        <h1>User List</h1>
        <Table striped bordered hover responsive>
          <thead> {/* table header */}
            <tr>
              <th>#</th>
              <th>First Name</th>
              <th>Last Name</th>
              <th>Email</th>
              <th>Is Admin</th>
              <th>Edit/Delete</th>
            </tr>
          </thead>
          <tbody>
            {users.map(
              (user, idx) => (
                // table row key is index
                <tr key={idx}>
                    {/* table cells in each row */}
                  <td>{idx + 1}</td>
                  <td>{user.name}</td>
                  <td>{user.lastName}</td>
                  <td>{user.email}</td>
                  <td>
                    {/* display diff thing depending if admin */}
                    {user.isAdmin ? <i className="bi bi-check-lg text-success"></i> : <i className="bi bi-x-lg text-danger"></i>}
                  </td>
                  <td>
                    <LinkContainer to={`/admin/edit-user/${user._id}`}>
                      <Button className="btn-sm">
                        <i className="bi bi-pencil-square"></i>
                      </Button>
                    </LinkContainer>
                    {" / "}
                    <Button
                      variant="danger"
                      className="btn-sm"
                      onClick={() => deleteHandler(user._id)}
                    >
                      <i className="bi bi-x-circle"></i>
                    </Button>
                  </td>
                </tr>
              )
            )}
          </tbody>
        </Table>
      </Col>
    </Row>
  );
};

export default AdminUsersPageComponent;

